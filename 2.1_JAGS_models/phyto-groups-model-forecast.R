model{
  ...
  for(i in 2:N){
    ...
    mu[i] <- mu_g[i] + mu_c[i] + mu_b[i] + mu_f[i]
    
    mu_g[k,j]~dnorm(mu_g[k,j-1],tau_proc_g)
    
    mu_c[k,j]~dnorm(mu_c[k,j-1],tau_proc_c) 
    
    mu_d[k,j]~dnorm(mu_d[k,j-1],tau_proc_d) 
    
    mu_f[k,j]~dnorm(mu_f[k,j-1],tau_proc_f)

  }
  
  #### Priors
  tau_proc_g ~ dgamma(0.001,0.001)
  tau_proc_c ~ dgamma(0.001,0.001)
  tau_proc_d ~ dgamma(0.001,0.001)
  tau_proc_f ~ dgamma(0.001,0.001)
