...
#' @param start_date start date for forecast
 ...
 load_noaa_forecast <- function(siteID, start_date){
      ...
      fpath <- file.path("00_Data_files","formatted_noaa")
...
#' 4. take 30 draws of random walk for each phyto driver for 7 days into future
#' 5. combine all the above in a data frame with the following columns:
#' date, column for each driver, ensemble member
#' 
#' @param start_date start date of forecast (today) in yyyy-mm-dd
#' @param model_id which model are you generating forecasts with?
#' 

noaa <- read_csv(file.path(paste0("./00_Data_files/formatted_noaa/met_",start_date,".csv")))
```
...
#'
#' @param fc_conv dataframe which holds all the driver data 
